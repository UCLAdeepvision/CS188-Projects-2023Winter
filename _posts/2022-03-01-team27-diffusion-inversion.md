...
1. learning-based methods, where we train encoders that approximate $$G^{-1}$$,
...
2. can accelerate sampling by taking more than $$1$$ timestep jump.
...
$$
\mathcal{L}(\mathbf{z}) = \mathcal{R}(\boldsymbol{\epsilon}_\theta(\mathbf{z}), \mathbf{x}) ,
$$

where $$\mathcal{R}$$ is the reconstruction loss and can include
...
With the loss function defined, we can simply perform optimization (e.g., SGD, Adam, etc.) with the gradient $$\frac{\partial \mathcal{L}}{\mathbf{z}}$$, iteratively updating $$\mathbf{z}$$ until convergence, at which point $$\hat{\mathbf{x}} = \boldsymbol{\epsilon}_\theta(\mathbf{z})$$ should be a good reconstruction of $$\mathbf{x}$$.
...
We use the Adam optimizer with learning rate $$0.0002$$ and default hyperparameters for $$72000$$ iterations with a batch size of $$64$$. Doing so takes approximately $$11$$ hours on a single 450W NVIDIA RTX 3090 with 24GB VRAM. We also clip the gradient entries to $$1.0$$ to improve model stability. We also set an EMA of $$0.9995$$ for the model parameters to further improve stability, which is less than DDPM and DDIM's $$0.9999$$ as they train for much more iterations. We do not use weight decay, as we found doing so decreases the output generation quality.
